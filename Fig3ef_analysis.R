...
glu_steadystate <- subset(fixed_OD_conc_glu, fixed_OD_conc_glu$Time_min >= 135)
glu_steadystate <- subset(glu_steadystate, glu_steadystate$Time_min <= 269)
...
lm_rep1_glu_conc <- lm(log(cellconc) ~ Time_hrs, data = glu_steadystate_rep1)
lm_rep2_glu_conc <- lm(log(cellconc) ~ Time_hrs, data = glu_steadystate_rep2)
lm_rep3_glu_conc <- lm(log(cellconc) ~ Time_hrs, data = glu_steadystate_rep3)
lm_rep4_glu_conc <- lm(log(cellconc) ~ Time_hrs, data = glu_steadystate_rep4)
...
lm_rep1_glu_od <- lm(log(OD600) ~ Time_hrs, data = glu_steadystate_rep1)
lm_rep2_glu_od <- lm(log(OD600) ~ Time_hrs, data = glu_steadystate_rep2)
lm_rep3_glu_od <- lm(log(OD600) ~ Time_hrs, data = glu_steadystate_rep3)
lm_rep4_glu_od <- lm(log(OD600) ~ Time_hrs, data = glu_steadystate_rep4)
#extract growth rate (slope) from linear models for each replicate and save as a vector
...
#trim data to include only within steady-state
suc_steadystate <- subset(fixed_OD_conc_suc, fixed_OD_conc_suc$Time_min >= 135)
suc_steadystate <- subset(suc_steadystate, suc_steadystate$Time_min <= 269)
#split steady-state data by rep
...
#cell conc growth rate estimate
#fit growth rates using linear regression of log transformed cell conc vs. time in hours
lm_rep1_suc_conc <- lm(log(cellconc) ~ Time_hrs, data = suc_steadystate_rep1)
lm_rep2_suc_conc <- lm(log(cellconc) ~ Time_hrs, data = suc_steadystate_rep2)
lm_rep3_suc_conc <- lm(log(cellconc) ~ Time_hrs, data = suc_steadystate_rep3)
lm_rep4_suc_conc <- lm(log(cellconc) ~ Time_hrs, data = suc_steadystate_rep4)
#extract growth rate (slope) from linear models for each replicate and save as a vector
...
#OD growth rate estimate
#fit growth rates using linear regression of log transformed od vs. time in hours
lm_rep1_suc_od <- lm(log(OD600) ~ Time_hrs, data = suc_steadystate_rep1)
lm_rep2_suc_od <- lm(log(OD600) ~ Time_hrs, data = suc_steadystate_rep2)
lm_rep3_suc_od <- lm(log(OD600) ~ Time_hrs, data = suc_steadystate_rep3)
lm_rep4_suc_od <- lm(log(OD600) ~ Time_hrs, data = suc_steadystate_rep4)
#extract growth rate (slope) from linear models for each replicate and save as a vector
...
#
Figure3f <- ggplot(data = fixed_OD_conc_suc) +geom_rect(aes(xmin = 135/60, xmax = 269/60, ymin = 1E5, ymax = 5E9), colour = "gray80", fill = "gray80") +geom_point(aes(y = cellconc, x = Time_hrs), fill = "black", size = 5, alpha = 0.6) +geom_point(aes(y = OD600*1E9, x = Time_hrs), size = 5, alpha = 0.6, colour = "firebrick", fill = "firebrick") +geom_point(data = expected_OD_suc, aes(y = exp_y, x = exp_x), colour = "firebrick", fill = "white", size = 5, alpha = 0.4, shape = 21) +geom_line(data = fixed_conc_suc, aes(y = cellconc, x = Time_hrs, group = as.factor(Rep)), linetype = "solid") +geom_line(data = fixed_OD_suc, aes(y = OD600*1E9, x = Time_hrs, group = as.factor(Rep)), linetype = "solid", colour = "firebrick") +scale_x_continuous(limits = c(0, 9), breaks = seq(0, 8, 1), labels = c(0,"", 2,"", 4,"", 6, "", 8)) +scale_y_continuous(limits = c(1E5, 5E9), breaks = c(1E5, 2E5, 3E5, 4E5, 5E5, 6E5, 7E5, 8E5, 9E5, 1E6, 2E6, 3E6, 4E6, 5E6, 6E6, 7E6, 8E6, 9E6, 1E7, 2E7, 3E7, 4E7, 5E7, 6E7, 7E7, 8E7, 9E7, 1E8, 2E8, 3E8, 4E8, 5E8, 6E8, 7E8, 8E8, 9E8, 1E9, 2E9, 3E9, 4E9, 5E9), labels = c(expression("1x10"^5), "", "", "", "", "", "", "", "", expression("1x10"^6),"", "", "", "", "", "", "", "", expression("1x10"^7), "", "", "", "", "", "", "", "", expression("1x10"^8),"", "", "", "", "", "", "", "", expression("1x10"^9), "", "", "", ""), trans = "log", name = "Cell concentration (cells/ml)", sec.axis = sec_axis(trans = ~.*1, breaks = c(1E6, 1E7, 1E8, 1E9), labels = c(0.001, 0.01, 0.1, 1),name = expression("OD"[600]))) + xlab('Time (hours)') + theme(legend.position = "none", strip.background = element_blank(), strip.text = element_blank(), panel.background = element_blank(), axis.line.x = element_line(colour = "black"), axis.line.y = element_line(colour = "black"), axis.ticks = element_line(colour = "black"), panel.grid.minor = element_blank(), panel.grid.major = element_blank(), axis.text.y = element_text(colour = "black", size = 24), axis.text.y.right = element_text(colour = "firebrick", size = 24), axis.text.x = element_text(colour = "black", size = 24), axis.title.y = element_text(size = 32, angle = 90), axis.title.y.right = element_text(size = 32, angle = 90, colour = "firebrick"), axis.title.x = element_text(size = 32))
Figure3f